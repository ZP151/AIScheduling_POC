...
{
    public class ClassroomConflictConstraint : IConstraint
    {
        public int Id { get; } = 2;
        public string Name { get; } = "Classroom Time Conflict";
        public string Description { get; } = "A classroom cannot be used for two different courses at the same time";
        ...
        public (double Score, List<SchedulingConflict> Conflicts) Evaluate(SchedulingSolution solution)
        {
            ...
            // 按教室ID分组
            var classroomGroups = solution.Assignments.GroupBy(a => a.ClassroomId);

            foreach (var group in classroomGroups)
            {
                var classroomId = group.Key;
                        ...
                        if (a1.TimeSlotId == a2.TimeSlotId)
                        {
                            ...
                            if (weekOverlap)
                            {
                                conflicts.Add(new SchedulingConflict
                                {
                                    ...
                                    Description = $"Classroom {a1.ClassroomName} has two courses scheduled at the same time: {a1.SectionCode} and {a2.SectionCode}",
