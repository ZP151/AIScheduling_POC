...
{
    /// <summary>
    /// Genetic algorithm implementation for optimizing scheduling solutions
    /// </summary>
    public class GeneticAlgorithm : ISchedulingAlgorithm
    {
        ...
        public GeneticAlgorithm(
            ...
            ILogger<GeneticAlgorithm> logger,
        ...
        {
            throw new NotSupportedException("Genetic algorithm should not be used to generate initial solutions directly.");
        }

        /// <summary>
        /// Optimize an existing scheduling solution using genetic algorithm
        /// </summary>
        public async Task<SchedulingSolution> OptimizeSolutionAsync(SchedulingSolution solution, CancellationToken cancellationToken = default)
        {
            ...
            {
                var newSolution = GenerateNeighbor(solution);
            ...
            for (int generation = 0; generation < maxGenerations && !cancellationToken.IsCancellationRequested; generation++)
            {
                ...
                while (newPopulation.Count < populationSize)
                {
                    ...
                    // Local optimization with simulated annealing for some individuals
                    if (_random.NextDouble() < 0.2) // 20% chance to apply SA
                    {
                        try
                        {
                            offspring = await _simulatedAnnealing.OptimizeSolutionAsync(offspring, cancellationToken);
                        }
                        catch (Exception ex)
                        {
                ...
                // Periodically check for early convergence
                if (generation % 10 == 0 && generation > 0)
                {
                    double populationDiversity = CalculatePopulationDiversity(population);
                    _logger.LogDebug($"Generation {generation}: Population diversity: {populationDiversity:F4}");
            ...
            _logger.LogInformation($"Genetic algorithm optimization complete. Best score: {bestEvaluation.Score:F4}, Feasible: {bestEvaluation.IsFeasible}");
            return bestSolution;
        }
        ...
        private void MutateTeacher(SchedulingSolution solution)
        {
            ...
            // Find suitable teachers for this section
            var suitableTeachers = _problem.TeacherCoursePreferences
            ...
            // If no preferred teachers, use any teacher
            if (suitableTeachers.Count <= 1)
            {
                suitableTeachers = _problem.Teachers.ToList();
            ...
            int currentTeacherIdx = suitableTeachers.FindIndex(t => t.Id == assignment.TeacherId);
            ...
            do
            {
                newTeacherIdx = _random.Next(suitableTeachers.Count);
            } while (newTeacherIdx == currentTeacherIdx);

            var newTeacher = suitableTeachers[newTeacherIdx];
